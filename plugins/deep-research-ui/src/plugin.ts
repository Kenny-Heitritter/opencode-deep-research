...
export const DeepResearchUIPlugin: Plugin = async () => {
  return {
    tool: {
      "deep_research_ui": tool({
        description: "Start a deep research run with the specified parameters",
        args: {
          query: tool.schema
            .string()
        ...
        async execute(args: any, context: ToolContext) {
          ...
          const response = {
            ...
            message: `Deep research run ${runId} initiated with effort level ${args.effort}`,
          };
